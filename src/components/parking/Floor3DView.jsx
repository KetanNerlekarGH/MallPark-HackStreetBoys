...
/**
 * Creates Dynamic 3D Mall Entrance Marquee Signage & Architectural Roof Structure
 */
function createDynamicMallStructure(selectedMall) {
  const mallGroup = new THREE.Group();
  const mallName = selectedMall?.name || "Phoenix Marketcity";
  const primaryColor = selectedMall?.theme?.primary || "#8b5cf6";
  const style = selectedMall?.architecturalStyle || "Multi-Wing Atrium";

  const colorHex = parseInt(primaryColor.replace("#", ""), 16) || 0x8b5cf6;

  // 1. Top Front Entrance Marquee Billboard Box
  const signBoxGeo = new THREE.BoxGeometry(28.0, 2.5, 0.8);
  signBoxGeo.translate(0, 28.5, 18.0);
  ...
  mallGroup.add(signBox);

  // Illuminated Marquee Banner Front Facing
  const bannerGeo = new THREE.BoxGeometry(27.0, 1.8, 0.1);
  bannerGeo.translate(0, 28.5, 18.42);

  // Create Canvas Texture with exact Mall Name Text
  ...
  tCtx.fillText(`🏬 ${mallName.toUpperCase()} · SMART PARKING`, 512, 64);
  ...
  mallGroup.add(new THREE.Mesh(bannerGeo, bannerMat));

  // 2. Custom Architectural Roof Canopy & Dome depending on Mall Name/Style
  if (mallName.toLowerCase().includes("pavilion") || style.toLowerCase().includes("circular")) {
    // Signature Circular Glass Atrium Dome for Pavilion Mall
    const domeGeo = new THREE.CylinderGeometry(8.0, 8.0, 3.5, 32);
    domeGeo.translate(0, 29.5, -2.0);
    ...
    mallGroup.add(new THREE.Mesh(domeGeo, domeMat));
  } else if (mallName.toLowerCase().includes("amanora") || mallName.toLowerCase().includes("seasons") || style.toLowerCase().includes("twin")) {
    // Twin Skybridge Connector Beam for Amanora / Seasons Mall
    const bridgeGeo = new THREE.BoxGeometry(38.0, 2.0, 4.0);
    bridgeGeo.translate(0, 29.0, -14.0);
    ...
    mallGroup.add(new THREE.Mesh(bridgeGeo, bridgeMat));
  } else {
    // Phoenix Marketcity / Standard Multi-Wing Grand Facade Pillars
    [-18.5, 18.5].forEach((px) => {
      const wingGeo = new THREE.BoxGeometry(3.5, 27.0, 4.0);
      wingGeo.translate(px, 13.5, -2.0);
      const wingMat = new THREE.MeshStandardMaterial({
        color: colorHex,
        metalness: 0.7,
        roughness: 0.3,
      });
      mallGroup.add(new THREE.Mesh(wingGeo, wingMat));
    });
  }
...
export default function Floor3DView({
  ...
  useEffect(() => {
    ...
    const bgColor = isDark ? "#06040a" : "#f1f5f9";
    ...
    // Add Dynamic Mall Facade Marquee Structure matching active Mall
    const mallStructure = createDynamicMallStructure(selectedMall);
    scene.add(mallStructure);
